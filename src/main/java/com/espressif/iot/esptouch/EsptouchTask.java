...
public class EsptouchTask implements IEsptouchTask {
    ...
    public EsptouchTask(String apSsid, String apBssid, String apPassword) {
        this(apSsid, apBssid, apPassword, null);
    }
     ...
     * @param apPassword the Ap's password
     */
    public EsptouchTask(byte[] apSsid, byte[] apBssid, byte[] apPassword) {
        this(apSsid, apBssid, apPassword, null);
    }

    private EsptouchTask(String apSsid, String apBssid, String apPassword, ITouchEncryptor encryptor) {
        ...
        init(ssid, bssid, password, encryptor);
    }

    private EsptouchTask(byte[] apSsid, byte[] apBssid, byte[] apPassword, ITouchEncryptor encryptor) {
    ...
    private void init(TouchData ssid, TouchData bssid, TouchData password, ITouchEncryptor encryptor) {
        _mParameter = new EsptouchTaskParameter();
        _mEsptouchTask = new __EsptouchTask(ssid, bssid, password, encryptor, _mParameter);
    }
